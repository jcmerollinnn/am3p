...
def load_raw(file_path):
    ...
    if ext == '.npy':
        ...
        return (cv2.imread(file_path)[:, :, ::-1].copy() * (1 / 255.0)).astype(np.float32)
    else:
...
class Retoucher(QWidget):
    def __init__(self, input_folder, username):
        ...
        self.fns = [*filter(lambda x: x.endswith('.npy') or x.endswith('.jpg') or x.endswith('.jpeg'), sorted(os.listdir(self.srcdir)))]
    ...
    def save(self):
        fn = self.fns[self.index][:-len('.jpg')]
